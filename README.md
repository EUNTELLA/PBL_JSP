...
`run.bat`은 `JAVA_HOME`이 JDK 20이면 그대로 사용하고, 없으면 일반적인 설치 경로에서 JDK 20을 찾습니다. Tomcat은 `CATALINA_HOME`이 설정되어 있으면 그대로 사용하고, 없으면 프로젝트 폴더, `C:\Program Files`, 사용자 폴더, 다운로드 폴더에서 Tomcat 9를 찾습니다.
