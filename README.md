...
- **게시글 검색** - 제목, 내용, 작성자 기준 검색 기능 제공
- **페이징** - 게시글 목록을 10개 단위로 나누어 표시
- **상세보기** - 목록에서 제목을 클릭해 게시글 본문과 댓글 확인
- **댓글 수 표시** - 목록 제목 옆에 댓글 개수 표시
- **모바일 기준 UI** - 작은 화면에서 메뉴, 목록, 상세보기 영역을 모바일 앱 형태로 정리
- **JOIN 목록 조회** - 게시글 목록에서 작성자 이름과 댓글 수를 `LEFT JOIN`으로 함께 조회
...
- 게시글 목록은 `feed`, `user`, `reply`를 `LEFT JOIN`하여 작성자 이름과 댓글 수를 한 번에 조회합니다.
...
| `jsp/feedView.jsp` | 게시글 상세보기, 댓글 목록 출력 |
...
12. 댓글 작성
13. 본인 댓글 삭제
14. 정보수정 화면에서 회원 정보 수정
15. 정보수정 화면에서 회원탈퇴
16. 로그아웃 후 메뉴가 `로그인`, `회원가입`으로 바뀌는지 확인
